...
typedef enum QITEM_status_en {
	QITEM_WAITING = 0x00,	/* State assigned to any item added to queue (initial state). */
	QITEM_SENT,				/* State assigned to #QITEM_SIGNATURE_REQUEST item when it is sent out. */
	QITEM_DONE,				/* State assigned to #QITEM_SIGNATURE_REQUEST item when request failed or succeeded. */
} QITEM_status;
	...
	while (true) {
		...
		if (ProtectedQueue_popFront(ctx->signer_queue, (void**) &item) != 0) {
			if (item->type == QITEM_CLOSE_FILE) {
				 ...
				 * thread quickly.
				 */
				rsksictxCloseAllPendingBlocksWithoutSignature(ctx, "Signing not finished due to sudden closure of lmsig_ksi-ls12 module.");
